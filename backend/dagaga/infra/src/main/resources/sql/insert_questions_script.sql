...
-- 자기소개
INSERT INTO question_bank (category, question_text, example_answer, order_index) VALUES
...
('자기소개', '한국 생활은 어떤 점이 가장 어려웠나요?', '가장 어려운 점은 한국어로 의사소통하는 거예요.', 5),
('자기소개', '한국에서 자주 가는 곳이 있나요?', '저는 집 근처 마트와 공원에 자주 가요.', 6),
('자기소개', '한국에서 주로 어떤 일을 하시나요?', '저는 아이를 돌보고 집안일을 하면서 지내요.', 7),
('자기소개', '취미나 좋아하는 활동이 있나요?', '저는 음악 듣는 것을 좋아해요.', 8),
('자기소개', '한국에서 가장 좋아하는 음식은 무엇인가요?', '저는 김치찌개를 좋아해요.', 9),
('자기소개', '앞으로 한국어 공부 목표가 있나요?', '아이 학교 생활에서 필요한 말을 자연스럽게 하고 싶어요.', 10);
