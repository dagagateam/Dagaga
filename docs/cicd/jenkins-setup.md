...
```bash
# 1. Jenkins 전용 Dockerfile 작성 (Docker CLI 설치)
...
1. `http://i14b110.p.example.io:9090` 접속.
...
2. **GitLab Connections**:
   - **Connection name**: 자유롭게 입력 (예: `GitLab-Example`)
   - **GitLab host URL**: 반드시 **GitLab 서버의 기본 주소** (`https://lab.example.io`)만 입력하세요.
   - **Credentials**: 위에서 등록한 `gitlab-token` 선택.
3. **Test Connection**: 'Success'가 뜨는지 확인.

---

## 5. Jenkins Job 소스 코드 연동 (Git SCM)

Job 설정의 **Pipeline** 섹션에서 실제 코드를 가져오기 위한 인증을 설정합니다.

1. **Definition**: `Pipeline script from SCM` 선택.
2. **SCM**: `Git` 선택.
3. **Repository URL**: `https://lab.example.com/s14-webmobile2-sub1/S14P11B110.git` (끝에 **.git**을 붙여주세요!)
4. **Credentials** (에러 발생 시): [Add] -> [Jenkins] 클릭하여 추가.
   - **Kind**: `Username with password` (**중요: GitLab API Token이 아닙니다!**)
   - **Username**: GitLab 아이디
   - **Password**: 발급받은 GitLab Personal Access Token (**비밀번호가 아닙니다!**)
   - **ID**: `gitlab-git-auth`
5. **Branches to build**: `*/cicd-test` 입력.
6. **Script Path**: `Jenkinsfile` (기본값).

---

## 6. GitLab Webhook 설정 (자동 빌드)
...
`http://i14b110.p.example.io:9090` 접속이 안 될 경우 다음을 확인하세요.
...
6. **빌드 성공(초록색)인데 스테이지가 안 보임 (Build skipped)**:
   ...
   - **해결**: `develop` 브랜치에 소량을 수정해서 **직접 Push** 해보거나, `Jenkinsfile`에서 `changeset` 조건을 잠시 지우고 테스트해 보세요. (현재 `Jenkinsfile`은 `origin/cicd-test`와 같이 접두어가 붙은 브랜치도 인식하도록 개선되었습니다.)

7. **`docker: not found` 에러**:
   - **원인**: Jenkins 공식 이미지에는 `docker` 명령어(CLI)가 포함되어 있지 않습니다.
   - **해결**: 위 **1번 섹션**의 `Dockerfile.jenkins`를 사용하여 직접 이미지를 빌드하고 실행해야 합니다. (`--build` 옵션으로 재시작 하세요.)

### [추천] Nginx를 통한 Jenkins 접속 (보안 강화)

9090 포트를 외부에 직접 여는 대신, Nginx를 통해 `i14b110.p.example.io/jenkins`로 접속하도록 설정할 수 있습니다. (이 경우 AWS에서 9090을 열지 않아도 됩니다.)
