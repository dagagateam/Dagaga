...
```bash
# SSH 접속
ssh -i <your-key>.pem ubuntu@i14b110.p.example.io
...
아래 내용을 `/etc/nginx/sites-available/i14b110.p.example.io` 파일에 **전체 복사 붙여넣기** 하세요.
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/i14b110.p.example.io/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/i14b110.p.example.io/privkey.pem; # managed by Certbot
