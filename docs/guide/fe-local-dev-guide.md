...
- **백엔드**: EC2 서버 사용 (`https://i14b110.p.example.io`)
...
→ https://i14b110.p.example.io/api/users
```
...
server: {
  proxy: {
    '/api': {
      target: 'https://i14b110.p.example.io',  // EC2 백엔드 주소
      changeOrigin: true,
      secure: false,
    },
  },
...
useEffect(() => {
  axios
    .get("/api/health") // 또는 실제 API 엔드포인트
...
1. **EC2 백엔드 상태 확인**
   ...
   ```bash
   curl https://i14b110.p.example.io/api/health
   ```
